            ...
            <div class="container mx-auto p-4">
                ...
                @auth
                    ...
                    <div x-data="{ open:false }" id="applicant-form">
                        ...
                        <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center bg-[rgba(0,0,0,0.5)]">
                            <div @click.away="open=false" class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                                ...
                                <form method="POST" action="{{route('applicant.store', $job->id)}}" enctype="multipart/form-data">
                                    ...
                                    <x-inputs.text-area id="message" name="message" label="Message" rows="3" />
        ...
        <aside class="bg-white rounded-lg shadow-md p-3">
            ...
            @else
                <form method="POST" action="{{auth()->user()->bookmarkedJobs()->where('job_id', $job->id)->exists() ? route('bookmarks.destroy', $job->id) : route('bookmarks.store', $job->id)}}" class="mt-10">
                    ...
                    @method('DELETE')
                        <button class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 w-full px-4 rounded-full flex items-center justify-center">
                            <i class="fas fa-bookmark mr-3"></i> Remove Bookmark
                        </button>
                    @else
                        <button class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 w-full px-4 rounded-full flex items-center justify-center">
