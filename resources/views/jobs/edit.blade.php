<x-layout>
    <x-slot name="title">Edit Job Lisiting</x-slot>
    <div class="bg-white mx-auto p-8 rounded-lg shadow-md w-full md:max-w-3xl">
        ...
        <form method="POST" action="{{route('jobs.update', $job->id)}}" enctype="multipart/form-data">
            ...
            <x-inputs.text id='salary' name='salary' type='number' label='Salary' placeholder='90000'  :value="old('salary', $job->salary)" />

            <x-inputs.text id='requirements' name='requirements' label='Requirements' placeholder='Bachelor's degree in Computer Science' :value="old('requirements', $job->requirements)" />

            <x-inputs.text id='benefits' name='benefits' label='Benefits' placeholder='Health insurance, 401k, paid time off' :value="old('benefits', $job->benefits)" />     
            ...
            <x-inputs.text id='company_description' name='company_description' label='Company Description' placeholder=' Enter company description' :value="old('company_description', $job->company_description)" />

            <x-inputs.text id='company_website' name='company_website' label='Company Website' placeholder='Enter website' type='url' :value="old('company_website', $job->company_website)" />
            ...
            <x-inputs.text id='contact_email' name='contact_email' label='Contact Email' placeholder='Enter email' :value="old('contact_email', $job->contact_email)" />
